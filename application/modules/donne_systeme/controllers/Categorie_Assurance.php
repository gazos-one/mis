...
class Categorie_Assurance  extends CI_Controller{
    ...
    public function index()
    {
      ...
      $resultat=$this->Model->getRequete('SELECT ID_CATEGORIE_ASSURANCE, syst_categorie_assurance.STATUS, IF(syst_categorie_assurance.STATUS=1, "Actif", "Innactif") AS NSTATUS, syst_categorie_assurance.DESCRIPTION AS ADESC, DROIT_AFFILIATION, COTISATION_MENSUELLE, PLAFOND_COUVERTURE_HOSP_JOURS, PLAFOND_ANNUEL, PLAFOND_LUNETTE, PLAFOND_MONTURES, PLAFOND_PROTHESES_DENTAIRES,PLAFOND_PHARMACEUTICAL,PLAFOND_CESARIENNE, PLAFOND_SCANNER, syst_regime_assurance.DESCRIPTION AS RASSU FROM syst_categorie_assurance JOIN syst_regime_assurance ON syst_regime_assurance.ID_REGIME_ASSURANCE = syst_categorie_assurance.ID_REGIME_ASSURANCE');
      $tabledata=array();
      ...
      foreach ($resultat as $key) 
         {
          ...
          $tabplafond = '<table class="table">';
          $tabplafond .= '<tr><td>Plafond Annuel</td><td>'.number_format($key['PLAFOND_ANNUEL'],0,","," ").'</td>';
          $tabplafond .= '<td>Plafond hospitalisation</td><td>'.number_format($key['PLAFOND_COUVERTURE_HOSP_JOURS'],0,","," ").'</td></tr>';
          $tabplafond .= '<tr><td>Plafond Lunette médicale</td><td>'.number_format($key['PLAFOND_LUNETTE'],0,","," ").'</td>';
          $tabplafond .= '<td>Plafond Montures</td><td>'.number_format($key['PLAFOND_MONTURES'],0,","," ").'</td></tr>';
          $tabplafond .= '<tr><td>Plafond Prothèses dentaires</td><td>'.number_format($key['PLAFOND_PROTHESES_DENTAIRES'],0,","," ").'</td>';
          $tabplafond .= '<td>Plafond Pharmaceutical</td><td>'.number_format($key['PLAFOND_PHARMACEUTICAL'],0,","," ").'</td></tr>';
          $tabplafond .= '<tr><td>Plafond Cesarienne</td><td>'.number_format($key['PLAFOND_CESARIENNE'],0,","," ").'</td>';
          $tabplafond .= '<td>Plafond Scanner</td><td>'.number_format($key['PLAFOND_SCANNER'],0,","," ").'</td></tr>';
          ...
          // $colabo[]='<div class="text-right">'.number_format($key['PLAFOND_COUVERTURE_HOSP_JOURS'],0,","," ").'</div>';
...
<div class="dropdown">
       ...
       $tabledata[]=$colabo;
        ...
        $this->table->set_heading(array('Regime Assurance','Intitulé','Droit d\'affiliation','Cotisation mensuelle','Nb Membres','Status','D&eacute;tails'));
  ...
  $PLAFOND_ANNUEL=$this->input->post('PLAFOND_ANNUEL');
  $PLAFOND_LUNETTE=$this->input->post('PLAFOND_LUNETTE');
  $PLAFOND_MONTURES=$this->input->post('PLAFOND_MONTURES');
  $PLAFOND_PROTHESES_DENTAIRES=$this->input->post('PLAFOND_PROTHESES_DENTAIRES');
  $PLAFOND_PHARMACEUTICAL=$this->input->post('PLAFOND_PHARMACEUTICAL');

  $PLAFOND_CESARIENNE=$this->input->post('PLAFOND_CESARIENNE');
  $PLAFOND_SCANNER=$this->input->post('PLAFOND_SCANNER');
  ...
  $this->form_validation->set_rules('PLAFOND_COUVERTURE_HOSP_JOURS', 'Plafond couverture', 'required');
  $this->form_validation->set_rules('PLAFOND_ANNUEL', 'Plafond annuel', 'required');
  $this->form_validation->set_rules('PLAFOND_LUNETTE', 'Plafond Lunette', 'required');
  $this->form_validation->set_rules('PLAFOND_MONTURES', 'Plafond Monture', 'required');
  $this->form_validation->set_rules('PLAFOND_PROTHESES_DENTAIRES', 'Plafond Prothese dentaire', 'required');
  $this->form_validation->set_rules('PLAFOND_PHARMACEUTICAL', 'Plafond Pharmaceutical', 'required');
  $this->form_validation->set_rules('PLAFOND_CESARIENNE', 'Plafond cesarienne', 'required');
  $this->form_validation->set_rules('PLAFOND_SCANNER', 'Plafond scanner', 'required');
  ...
  $this->form_validation->set_rules('STRUCT_C', 'Structure C', 'required');
   ...
   if ($this->form_validation->run() == FALSE){
    ...
    $datacatass=array(
                       ...
                       'PLAFOND_LUNETTE'=>$PLAFOND_LUNETTE,
                       'PLAFOND_MONTURES'=>$PLAFOND_MONTURES,
                       'PLAFOND_PROTHESES_DENTAIRES'=>$PLAFOND_PROTHESES_DENTAIRES,
                       'PLAFOND_PHARMACEUTICAL'=>$PLAFOND_PHARMACEUTICAL,
                       'PLAFOND_CESARIENNE'=>$PLAFOND_CESARIENNE,
                       'PLAFOND_SCANNER'=>$PLAFOND_SCANNER,
  ...
  $this->form_validation->set_rules('COTISATION_MENSUELLE', 'Cotisation Mensuele', 'required');
  $this->form_validation->set_rules('PLAFOND_COUVERTURE_HOSP_JOURS', 'Plafond couverture', 'required');
  $this->form_validation->set_rules('PLAFOND_ANNUEL', 'Plafond couverture', 'required');
  $this->form_validation->set_rules('PLAFOND_LUNETTE', 'Plafond couverture', 'required');
  $this->form_validation->set_rules('PLAFOND_MONTURES', 'Plafond couverture', 'required');
  $this->form_validation->set_rules('PLAFOND_PROTHESES_DENTAIRES', 'Plafond couverture', 'required');
  $this->form_validation->set_rules('PLAFOND_PHARMACEUTICAL', 'Plafond Pharmaceutical', 'required');
  $this->form_validation->set_rules('PLAFOND_CESARIENNE', 'Plafond Cesarienne', 'required');
  $this->form_validation->set_rules('PLAFOND_SCANNER', 'Plafond Scanner', 'required');
  $this->form_validation->set_rules('ID_REGIME_ASSURANCE', 'Regime assurance', 'required');
