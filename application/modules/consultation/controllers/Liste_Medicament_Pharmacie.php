   ...
   public function index($value='')
   {
    ...
    $crit=" AND consultation_medicament.STATUS_PAIEMENT = ".$STATUS_PAIEMENT."";
      ...
      $critaire = '';
       ...
       $groupby=' GROUP BY consultation_pharmacie.DESCRIPTION, consultation_pharmacie.ID_PHARMACIE';
              ...
              <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">Fermer</button>
                  ...
                  <div class="dropdown ">
                            ...
                            <li><a class="dropdown-item" href="#" onclick="getdetail('.$key->ID_PHARMACIE.')"> Apercus </a> </li>
    ...
    $data['ANNEE']=$critaire;
     ...
     //// function pour les details des medicaments prise
    ...
    $crit=" AND consultation_medicament.STATUS_PAIEMENT = ".$STATUS_PAIEMENT."";
      ...
      $query_principal='SELECT consultation_medicament_details.ID_CONSULTATION_MEDICAMENT_DETAILS,membre_membre.ID_MEMBRE,membre_membre.NOM, membre_membre.PRENOM, DATE_CONSULTATION, NUM_BORDERAUX, DESCRIPTION, MONTANT_UNITAIRE_SANS_TAUX AS PU, QUANTITE,consultation_medicament.TYPE_AFFILIE,consultation_medicament_details.POURCENTAGE, consultation_medicament.ID_CONSULTATION_MEDICAMENT, MONTANT_TOTAL_ACHAT as PT, MONTANT_A_PAYE_MIS AS PRIX_MIS FROM `consultation_medicament` JOIN membre_membre ON membre_membre.ID_MEMBRE = consultation_medicament.ID_MEMBRE JOIN consultation_medicament_details ON consultation_medicament_details.ID_CONSULTATION_MEDICAMENT = consultation_medicament.ID_CONSULTATION_MEDICAMENT JOIN masque_medicament ON masque_medicament.ID_MEDICAMENT = consultation_medicament_details.ID_MEDICAMENT WHERE 1 '.$critaire .' '.$crit.' AND ID_PHARMACIE = '.$ID_PHARMACIE.' ';
       ...
       $var_search = !empty($_POST['search']['value']) ? $_POST['search']['value'] : null;
        ...
        $search = !empty($_POST['search']['value']) ? (' AND (membre_membre.NOM LIKE "%' . $var_search . '%" or membre_membre.PRENOM LIKE "%' . $var_search . '%" or NUM_BORDERAUX LIKE "%' . $var_search . '%" or DESCRIPTION LIKE "%' . $var_search . '%" )') :'';
      ...
      foreach ($resultat as $key) 
         {
          ...
          $membre=$this->Model->getOne('membre_membre',array('ID_MEMBRE'=>$key->TYPE_AFFILIE));
          $groupe=$this->Model->getRequeteOne('SELECT NOM_GROUPE from membre_groupe_membre  JOIN membre_groupe ON membre_groupe_membre.ID_GROUPE = membre_groupe.ID_GROUPE WHERE 1 and ID_MEMBRE='.$key->ID_MEMBRE.'');
          $chambr[]=$membre['NOM']." ".$membre['PRENOM']; 
          ...
          $chambr[]='
                  <div class="dropdown ">
                            ...
                            <span class="caret"></span></a>
                            <ul class="dropdown-menu dropdown-menu-right">
                            <li><a class="dropdown-item" href="'.base_url("consultation/Enregistrer_Medicament/modif_view/".$key->ID_CONSULTATION_MEDICAMENT_DETAILS."/".$ID_PHARMACIE."").'"> Modifier </a> </li>

                            <li><a class="dropdown-item" href="'.base_url("consultation/Enregistrer_Medicament/delete/".$key->ID_CONSULTATION_MEDICAMENT_DETAILS).'"><font color="red"> Supprimer </font> </a> </li>
    ...
    $critaire = ' AND DATE_FORMAT(DATE_CONSULTATION,"%Y") Like "%'.$year.'%"';
     $totals=$this->Model->getRequeteOne('SELECT COUNT(consultation_medicament.ID_CONSULTATION_MEDICAMENT) AS NOMBRE, SUM(consultation_medicament.MONTANT_TOTAL_ACHAT) AS MONTANT_TOTAL_ACHAT, SUM(MONTANT_A_PAYE_MIS) AS MONTANT_MIS FROM `consultation_medicament` LEFT JOIN consultation_pharmacie ON consultation_pharmacie.ID_PHARMACIE = consultation_medicament.ID_PHARMACIE  WHERE 1 AND  consultation_medicament.STATUS_PAIEMENT = '.$this->input->post('STATUS_PAIEMENT').'  '.$critaire );
