...
class Consomation_Famille extends CI_Controller
{
  ...
  public function Is_Connected()
    ...
    //   }
      ...
      $resultat=$this->Model->getRequete('SELECT membre_membre.ID_MEMBRE, NOM, PRENOM, membre_groupe.NOM_GROUPE FROM `membre_membre` JOIN membre_groupe_membre ON membre_groupe_membre.ID_MEMBRE = membre_membre.ID_MEMBRE LEFT JOIN membre_groupe ON membre_groupe.ID_GROUPE = membre_groupe_membre.ID_GROUPE WHERE `IS_AFFILIE` = 0 '.$condi5.' ');
      // '.$critaire.' '.$critaire2.'
      ...
      foreach ($resultat as $key) 
         {
          ...
          if ($MOIS != null) {
            $YDATE = $ANNEE.'-'.$MOIS;
            $condiy = ' AND DATE_CONSULTATION LIKE "'.$YDATE.'%" ';
            $data['MOIS'] = $this->input->post('MOIS');
          }
          else{
            $condiy = ' AND YEAR(DATE_CONSULTATION) = '.$ANNEE.' ';
            $data['MOIS'] = NULL;
          }
            ...
            $condiy = ' AND YEAR(DATE_CONSULTATION) = '.date('Y').' ';
            $ANNEE = date('Y');
            $data['ANNEE']=date('Y');
            $data['MOIS'] = NULL;
            // $critaire = ' AND DATE_FORMAT(DATE_CONSULTATION,"%Y") Like "%'.$ANNEE.'%"';
          ...
          $sommeaff=$this->Model->getRequeteOne('SELECT SUM(MONTANT_A_PAYER) AS MONTANT_A_PAYER_AF FROM `consultation_consultation` WHERE  ID_MEMBRE = '.$key['ID_MEMBRE'].' '.$condiy.'  ');
          $sommeayant=$this->Model->getRequeteOne('SELECT SUM(MONTANT_A_PAYER) AS MONTANT_A_PAYER_AYA FROM `consultation_consultation` WHERE  ID_MEMBRE != '.$key['ID_MEMBRE'].' AND TYPE_AFFILIE = '.$key['ID_MEMBRE'].' '.$condiy.' ');

          $sommeaffmed=$this->Model->getRequeteOne('SELECT SUM(MONTANT_A_PAYE_MIS) AS MONTANT_A_PAYE_MIS_AF FROM `consultation_medicament` WHERE  ID_MEMBRE = '.$key['ID_MEMBRE'].' '.$condiy.' ');
          $sommeayantmed=$this->Model->getRequeteOne('SELECT SUM(MONTANT_A_PAYE_MIS) AS MONTANT_A_PAYE_MIS_AYA FROM `consultation_medicament` WHERE  ID_MEMBRE != '.$key['ID_MEMBRE'].' AND TYPE_AFFILIE = '.$key['ID_MEMBRE'].' '.$condiy.'');

          // $dataplafond = $this->Model->getRequeteOne('SELECT syst_categorie_assurance.PLAFOND_ANNUEL, syst_categorie_assurance.PLAFOND_COUVERTURE_HOSP_JOURS,syst_categorie_assurance.PLAFOND_LUNETTE, syst_categorie_assurance.PLAFOND_MONTURES, syst_categorie_assurance.PLAFOND_PROTHESES_DENTAIRES, syst_categorie_assurance.PLAFOND_PHARMACEUTICAL, syst_categorie_assurance.PLAFOND_CESARIENNE, syst_categorie_assurance.PLAFOND_SCANNER FROM membre_membre JOIN membre_assurances ON membre_membre.ID_MEMBRE = membre_assurances.ID_MEMBRE JOIN syst_categorie_assurance ON syst_categorie_assurance.ID_CATEGORIE_ASSURANCE = membre_assurances.ID_CATEGORIE_ASSURANCE WHERE membre_membre.ID_MEMBRE = '.$key['ID_MEMBRE'].'');  
          


          $chambr=array();
          $M_CONSULTATION = $sommeaff['MONTANT_A_PAYER_AF'] + $sommeayant['MONTANT_A_PAYER_AYA'];
          $M_MEDICAMENT = $sommeaffmed['MONTANT_A_PAYE_MIS_AF'] + $sommeayantmed['MONTANT_A_PAYE_MIS_AYA'];
          ...
          // $chambr[]='<div class="text-right">'.number_format($sommeayant['MONTANT_A_PAYER_AYA'],0,' ',' ').'</div>'; 

          $chambr[]='<div class="text-right">'.number_format($M_MEDICAMENT,0,' ',' ').'</div>'; 
          $chambr[]='<div class="text-right">'.number_format($C_TOTAL,0,' ',' ').'</div>'; 
          // $chambr[]='<div class="text-right">'.number_format($dataplafond['PLAFOND_ANNUEL'],0,' ',' ').'</div>'; 
          $totalconsultation += $M_CONSULTATION;
          $totalmedicaament += $M_MEDICAMENT;
          // 
          // $chambr[]='<div class="text-right">'.number_format($M_MEDICAMENT,0,' ',' ').'</div>';
          // $chambr[]='<div class="text-right">'.number_format($C_TOTAL,0,' ',' ').'</div>';
          // $chambr[]='<div class="text-right">'.number_format($key['nb_medicaments'],0,' ',' ').'</div>';
       ...
       $tabledata[]=$chambr;
        ...
        $this->table->set_heading(array('Affili&eacute;','Groupe','M Consultation','M Pharmacie','Total'));
        
        $data['title'] = " Consomation";
        $data['stitle']=' Consomation';
        $data['chamb']=$tabledata;
        $data['totalconsultation']=$totalconsultation;
        $data['totalmedicaament']=$totalmedicaament;
        $data['groupe'] = $this->Model->getList('membre_groupe'); 
        $this->load->view('Consultation_Famille_Liste_View',$data);
