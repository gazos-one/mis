...
class Enregistrer_Medicament extends CI_Controller {
  ...
  public function __construct() {
    ...
    public function modif_view($id,$ID_PHARMACIE)
    {
    $data['id']=$id;
    ...
    $data['type_med'] = $this->Model->getList('syst_couverture_medicament'); 
    $data['affilie']= $this->Model->getList("membre_membre",array('IS_AFFILIE'=>0));
    $data['affilie2']= $this->Model->getList("membre_membre");

    $data['medicament']= $this->Model->getList("masque_medicament");

    //   $data['province'] = 
    $data['one']=$this->Model->getRequeteOne('SELECT consultation_medicament_details.ID_CONSULTATION_MEDICAMENT_DETAILS, consultation_medicament.ID_PHARMACIE,consultation_medicament.MEDECIN,membre_groupe.ID_GROUPE,consultation_medicament_details.ID_CONSULTATION_MEDICAMENT_DETAILS,consultation_medicament.ID_MEMBRE,membre_membre.CODE_PARENT,membre_membre.NOM, membre_membre.PRENOM,consultation_medicament.DATE_CONSULTATION,consultation_medicament_details.ID_COUVERTURE_MEDICAMENT,consultation_medicament_details.ID_MEDICAMENT, IF(aff.CODE_PARENT IS NULL, membre_membre.NOM, aff.NOM) AS ANOM, IF(aff.CODE_PARENT IS NULL, membre_membre.PRENOM, aff.PRENOM) AS APRENOM, DATE_CONSULTATION, NUM_BORDERAUX, DESCRIPTION, MONTANT_UNITAIRE_SANS_TAUX AS PU, QUANTITE, consultation_medicament_details.POURCENTAGE, membre_groupe.NOM_GROUPE, consultation_medicament.ID_CONSULTATION_MEDICAMENT,consultation_medicament_details.QUANTITE,consultation_medicament_details.MONTANT_UNITAIRE_SANS_TAUX, (consultation_medicament_details.QUANTITE * consultation_medicament_details.MONTANT_UNITAIRE_SANS_TAUX) AS PT, (consultation_medicament_details.QUANTITE * consultation_medicament_details.MONTANT_UNITAIRE_SANS_TAUX) * POURCENTAGE / 100 AS PRIX_MIS FROM `consultation_medicament` LEFT JOIN membre_membre ON membre_membre.ID_MEMBRE = consultation_medicament.ID_MEMBRE LEFT JOIN consultation_medicament_details ON consultation_medicament_details.ID_CONSULTATION_MEDICAMENT = consultation_medicament.ID_CONSULTATION_MEDICAMENT LEFT JOIN masque_medicament ON masque_medicament.ID_MEDICAMENT = consultation_medicament_details.ID_MEDICAMENT LEFT JOIN membre_membre aff ON aff.ID_MEMBRE = membre_membre.CODE_PARENT LEFT JOIN membre_groupe_membre ON membre_groupe_membre.ID_MEMBRE = membre_membre.ID_MEMBRE LEFT JOIN membre_groupe ON membre_groupe_membre.ID_GROUPE = membre_groupe.ID_GROUPE WHERE 1 AND ID_CONSULTATION_MEDICAMENT_DETAILS = '.$id.' AND ID_PHARMACIE = '.$ID_PHARMACIE.' '); 

    // print_r($data['one']);die();
    $this->load->view('Modif_Medicament_View',$data);
    }
...
echo $datas;
    ...
    {
    $commune= $this->Model->getRequeteOne("SELECT * FROM syst_categorie_assurance_medicament JOIN syst_couverture_medicament ON syst_couverture_medicament.ID_COUVERTURE_MEDICAMENT = syst_categorie_assurance_medicament.ID_COUVERTURE_MEDICAMENT JOIN syst_categorie_assurance ON syst_categorie_assurance.ID_CATEGORIE_ASSURANCE = syst_categorie_assurance_medicament.ID_CATEGORIE_ASSURANCE JOIN membre_assurances ON membre_assurances.ID_CATEGORIE_ASSURANCE = syst_categorie_assurance.ID_CATEGORIE_ASSURANCE WHERE 1 AND membre_assurances.ID_MEMBRE = ".$this->input->post('ID_MEMBRE')." AND syst_categorie_assurance_medicament.ID_COUVERTURE_MEDICAMENT = ".$this->input->post('ID_COUVERTURE_MEDICAMENT')." ORDER BY syst_categorie_assurance_medicament.`ID_CATEGORIE_MEDICAMENT` DESC LIMIT 1");
    ...
    public function getpourcentage2()
    {

      $val= $this->Model->getOne("consultation_medicament_details",array('ID_CONSULTATION_MEDICAMENT_DETAILS'=>$this->input->post('ID_CONSULTATION_MEDICAMENT_DETAILS')));

      $val2= $this->Model->getOne("consultation_medicament",array('ID_CONSULTATION_MEDICAMENT'=>$val['ID_CONSULTATION_MEDICAMENT']));

    $commune= $this->Model->getRequeteOne("SELECT * FROM syst_categorie_assurance_medicament JOIN syst_couverture_medicament ON syst_couverture_medicament.ID_COUVERTURE_MEDICAMENT = syst_categorie_assurance_medicament.ID_COUVERTURE_MEDICAMENT JOIN syst_categorie_assurance ON syst_categorie_assurance.ID_CATEGORIE_ASSURANCE = syst_categorie_assurance_medicament.ID_CATEGORIE_ASSURANCE JOIN membre_assurances ON membre_assurances.ID_CATEGORIE_ASSURANCE = syst_categorie_assurance.ID_CATEGORIE_ASSURANCE WHERE 1 AND membre_assurances.ID_MEMBRE = ".$val2['ID_MEMBRE']." AND syst_categorie_assurance_medicament.ID_COUVERTURE_MEDICAMENT = ".$this->input->post('ID_COUVERTURE_MEDICAMENT')." ORDER BY syst_categorie_assurance_medicament.`ID_CATEGORIE_MEDICAMENT` DESC LIMIT 1");
    ...
    // print_r($datas);
     ...
     public function delete($ID_CONSULTATION_MEDICAMENT_DETAILS)
     {
      

        $one= $this->Model->getOne('consultation_medicament_details',array('ID_CONSULTATION_MEDICAMENT_DETAILS'=>$ID_CONSULTATION_MEDICAMENT_DETAILS));

      
        $this->Model->delete('consultation_medicament_details',array('ID_CONSULTATION_MEDICAMENT_DETAILS'=>$ID_CONSULTATION_MEDICAMENT_DETAILS));

        

        $new_data = $this->Model->getRequeteOne('SELECT SUM((((MONTANT_UNITAIRE_SANS_TAUX * QUANTITE) * POURCENTAGE) /100 )) AS TOTAL_PORCENTAGE, SUM((MONTANT_UNITAIRE_SANS_TAUX * QUANTITE)) AS TOTAL_BRUT, SUM(((MONTANT_UNITAIRE_SANS_TAUX * QUANTITE) - (((MONTANT_UNITAIRE_SANS_TAUX * QUANTITE) * POURCENTAGE) /100 ))) AS PAYABLE FROM `consultation_medicament_details`  WHERE 1 AND ID_CONSULTATION_MEDICAMENT LIKE '.$one['ID_CONSULTATION_MEDICAMENT'].' ');

        $this->Model->update('consultation_medicament',array('ID_CONSULTATION_MEDICAMENT'=>$one['ID_CONSULTATION_MEDICAMENT']),array('MONTANT_TOTAL_ACHAT'=>$new_data['TOTAL_BRUT'],'MONTANT_A_PAYE_MIS'=>$new_data['TOTAL_PORCENTAGE'],'MONTANT_PAYER_AFFILIE'=>$new_data['PAYABLE']));
      ...
      public function update()
    {
        ...
        $this->Model->update('consultation_medicament_details',array('ID_CONSULTATION_MEDICAMENT_DETAILS'=>$ID_CONSULTATION_MEDICAMENT_DETAILS), $datas);
