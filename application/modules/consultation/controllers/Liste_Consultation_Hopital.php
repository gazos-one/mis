     ...
     // $listes=$this->Model->getRequete('');
      ...
      $query_principal='SELECT membre_membre.NOM, membre_membre.PRENOM,COALESCE(aff.NOM, membre_membre.NOM) AS ANOM, COALESCE(aff.PRENOM, membre_membre.PRENOM) AS APRENOM,COALESCE(aff.ID_MEMBRE, membre_membre.ID_MEMBRE) AS ID_MEMBRE, consultation_consultation.DATE_CONSULTATION, consultation_consultation.NUM_BORDERAUX, consultation_consultation.MONTANT_CONSULTATION, consultation_consultation.POURCENTAGE_C, consultation_consultation.POURCENTAGE_A, consultation_consultation.MONTANT_A_PAYER, membre_groupe.NOM_GROUPE FROM `consultation_consultation` JOIN membre_membre ON membre_membre.ID_MEMBRE = consultation_consultation.ID_MEMBRE LEFT JOIN membre_membre aff ON aff.ID_MEMBRE = membre_membre.CODE_PARENT LEFT JOIN membre_groupe_membre ON membre_groupe_membre.ID_MEMBRE = aff.ID_MEMBRE LEFT JOIN membre_groupe ON membre_groupe.ID_GROUPE = membre_groupe_membre.ID_GROUPE WHERE 1 and '.$crit_pyment.' AND ID_STRUCTURE = '.$id.'';
       ...
       $var_search = !empty($_POST['search']['value']) ? $_POST['search']['value'] : null;
        ...
        $search = !empty($_POST['search']['value']) ? (' AND membre_membre.NOM "%' . $var_search . '%")') :'';
      ...
      foreach ($resultat as $key) 
        ...
        $modaldet="<div class='modal fade' id='myModal".$key->ID_MEMBRE."' tabindex='-1' role='dialog' aria-labelledby='myModalLabel'>
  <div class='modal-dialog modal-xl' role='document'>
    <div class='modal-content'>
      <div class='modal-header'>
        
        <h4 class='modal-title' id='myModalLabel'>Consomation de ".$key->ANOM." ".$key->APRENOM."</h4>
    ...
    $modaldet .= "<table id='examples".$i."' class='table' style='width:100%'>
          <thead>
...
<th>Groupe</th>
    ...
    $modaldet .= '
     <tr>
      ...
      <td>'.number_format($cvaluea['MONTANT_A_PAYER'],0,',',' ').' ('.$cvaluea['POURCENTAGE_A'].'%)</td>
      <td>'.$cvaluea['NOM_GROUPE'].'</td>
      </tr>
    ';
    }
      

      $modaldet .="</tbody></table></div>
      <div class='modal-footer'>
        <button type='button' class='btn btn-default' data-dismiss='modal'>Fermer</button>
      </div>
    </div>
...
$(document).ready(function() {
    $('#examples".$i."').DataTable({
        paging: false,
        ordering: false,
        info: false,
        searching: true,
        dom: 'Bfrtip', // This is essential to show buttons
        buttons: [
        ...
        // $tableliste .= $modaldet."";
          ...
          $chambr[]=$i; 
          $chambr[]=$key->ANOM." ".$key->APRENOM." <button type='button' class='btn btn-primary btn-xs' data-toggle='modal' data-target='#myModal".$key->ID_MEMBRE."'>Historique Consomation</button>".$modaldet;
          ...
          $chambr[]=number_format($key->MONTANT_A_PAYER,0,',',' ');
        ...
        //                     <span class="caret"></span></a>
        //                     <ul class="dropdown-menu dropdown-menu-right">
        //                     <li><a class="dropdown-item" href="#" data-toggle="modal" data-target="#desactcat'.$key->ID_PHARMACIE.'"> Apercus </a> </li>
    ...
    public function apercu($id)
    {
      ...
      $data['groupe']=$this->Model->getOne('masque_stucture_sanitaire',array('ID_STRUCTURE '=>$id));
      $data['title']='';
