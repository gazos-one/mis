         ...
         <div class="card-body">
               ...
               <form id="FormData" action="<?php echo base_url()?>membre/Membre/add" method="POST" enctype="multipart/form-data">
                  ...
                  (Ajout nouveau emploi)
                          ...
                          <select class="form-control select2" name="ID_EMPLOI" id="ID_EMPLOI" onchange="emploie(this)" >
                          <option value="" >-- Sélectionner --</option>
                          <?php
                          ...
                          <!-- <select class="form-control" name="TEST_GROUPE" id="TEST_GROUPE"> <option value="" >-- Sélectionner --</option> 
                          <?php foreach ($agroupe as $key => $groupe) { ?> 
                            <option value="<?=$groupe['ID_GROUPE']?>"><?=$groupe['NOM_GROUPE']?></option> 
                            <?php } ?> 
                          </select> -->
                          <?php echo form_error('ID_EMPLOI', '<div class="text-danger">', '</div>'); ?>
                       </div>
                       <input type="hidden" name="NEWGROUPE" class="form-control" value="" id="NEWGROUPE">
                      ...
                      <input type="hidden" readonly="readonly" name="ID_AGENCE" class="form-control" value="0" id="ID_AGENCE"> 
...
function emploie(selectElement) {
  document.getElementById("ID_REGIME_ASSURANCE").selectedIndex = 1;
  changeassurance();

  var selectedValue = selectElement.value;
  var testGroupeSelect = document.getElementById("ID_GROUPE");

  // Réinitialiser la sélection du menu déroulant TEST_GROUPE
  testGroupeSelect.selectedIndex = 0;

  $.post('<?php echo base_url('membre/Membre/get_groupe')?>',
    {selectedValue: selectedValue},
    function(data) {
      $('#NEWGROUPE').val(data);

      // Mettre à jour la sélection dans le menu déroulant TEST_GROUPE avec nEWGROUPE
      testGroupeSelect.value = data;
    });

  // Parcourir les options du menu déroulant TEST_GROUPE
  for (var i = 0; i < testGroupeSelect.options.length; i++) {
    var option = testGroupeSelect.options[i];

    // Vérifier si la valeur de l'option correspond à la sélection de ID_EMPLOI
