...
class Rappel extends CI_Controller {
     ...
     public function index()
    {
        ...
        $this->load->view('Rappel_List_View');
     ...
     $search = !empty($_POST['search']['value']) ? (' AND (NOM LIKE "%' . $var_search . '%" or PRENOM LIKE "%' . $var_search . '%" or TELEPHONE LIKE "%' . $var_search . '%" or CNI LIKE "%' . $var_search . '%" or FIN_SUR_LA_CARTE LIKE "%' . $var_search . '%"  )') :'';
 
     $critaire = '';
     $order_by="  ORDER BY ID_MEMBRE";
     $groupby='GROUP BY ID_MEMBRE';

     $resultat="SELECT membre_membre.ID_MEMBRE,membre_carte_membre.FIN_SUR_LA_CARTE,membre_membre.TELEPHONE, membre_membre.ID_MEMBRE,CODE_AFILIATION, NOM, PRENOM, CNI, DATE_ADHESION, membre_membre.STATUS, IS_AFFILIE FROM `membre_membre` JOIN membre_carte_membre ON membre_membre.ID_MEMBRE = membre_carte_membre.ID_MEMBRE WHERE 1 and date_format(membre_carte_membre.FIN_SUR_LA_CARTE,'%Y-%m-%d') <='".date("Y-m-d")."'";
 
     $query_secondaire = $resultat . '  ' . $critaire . '  ' . $search . '  ' . $groupby . '  ' . $order_by . '   ' . $limit;
     $query_filter = $resultat . ' ' . $critaire . '  ' . $search. ' ' . $groupby;
     $fetch_client = $this->Model->datatable($query_secondaire);
      ...
      foreach ($fetch_client as $key) 
       ...
       "recordsTotal" => $this->Model->all_data($resultat),
