...
class Carte_New extends CI_Controller {
    ...
    // $information=$this->Model->getRequete('SELECT saisie_client.ID_CLIENT AS ID_CLIENT, NOM_CLIENT, PRENOM_CLIENT, NIF_CLIENT, RC_CLIENT, TEL_CLIENT, EMAIL_CLIENT, saisie_client.ENVOIE, saisie_client.ID_SOCIETE, saisie_client.STATUS AS STATUS, saisie_client.DATE_TIME AS DATE_TIME, TYPE_PERSONNE, CLIENT_PRENEUR_ASSURANCE,(select count(ID_ASSURANCE) from saisie_client_assurance where saisie_client_assurance.ID_CLIENT=saisie_client.ID_CLIENT) as ID_ASSURANCE,(select count(ID_CLIENT_AFFILIE) from saisie_client_affiliation where saisie_client_affiliation.ID_CLIENT=saisie_client.ID_CLIENT) AS ID_CLIENT_AFFILIE FROM `saisie_client` left join `saisie_client_assurance` on saisie_client_assurance.ID_CLIENT=saisie_client.ID_CLIENT left join `saisie_client_affiliation` on saisie_client_affiliation.ID_CLIENT=saisie_client.ID_CLIENT WHERE 1 GROUP BY NOM_CLIENT');
    ...
    $this->load->view('Carte_New_List_View', $data);
    }
    ...
    $ysetting = $this->Model->getRequeteOne('SELECT * FROM syst_config WHERE ID_CONFIG = 1');
     ...
     public function listing($DATE_ADHESION=0)
    {


  $query_principal = 'SELECT membre_carte.ID_CARTE, membre_carte.CODE_CARTE, membre_carte.ID_CATEGORIE_ASSURANCE, membre_carte.STATUS, membre_carte.DATE_DABUT_VALIDITE, membre_carte.DATE_FIN_VALIDITE, membre_membre.ID_MEMBRE, membre_membre.NOM, membre_membre.PRENOM, membre_membre.CNI, IF(membre_carte.STATUS = 1, "Actif", "Innactif") AS STATUS_CARTE FROM `membre_carte` JOIN membre_carte_membre ON membre_carte_membre.ID_CARTE = membre_carte.ID_CARTE JOIN membre_membre ON membre_membre.ID_MEMBRE = membre_carte_membre.ID_MEMBRE WHERE membre_membre.IS_AFFILIE = 0';
...
$order_by = '';
$search = !empty($_POST['search']['value']) ? (" AND (membre_membre.NOM  LIKE '%$var_search%' OR membre_membre.PRENOM  LIKE '%$var_search%' OR membre_membre.CNI LIKE '%$var_search%' OR membre_carte.CODE_CARTE LIKE '%$var_search%')") : '';
$order_column='';
...
foreach ($fetch_cov_frais as $row) {
    $post = array();
        // $post[] = $u++;
        $nban = $this->Model->getRequeteOne('SELECT count(*) AS NB_AYANT FROM `membre_carte_membre` WHERE ID_CARTE = '.$row->ID_CARTE.' ');
    ...
    $post[] = $row->CODE_CARTE;
    $post[] = $row->CNI;
    $newDatedebut = date("d-m-Y", strtotime($row->DATE_DABUT_VALIDITE));
    ...
    $post[] = '<div class="dropdown ">
                    ...
                    <li><a class="dropdown-item" href="#" onclick="modificationduree('.$row->ID_CARTE.')">Modifier la date de fin</a> </li>
                    </ul>
                  </div>';
...
echo json_encode($output);
    ...
    //           </button>
    //         </div>
    //         <form id="FormData" action="'.base_url().'membre/Carte_New/changedate" method="POST" enctype="multipart/form-data">
    //         <div class="modal-body">
    ...
    //                 <li><a class="dropdown-item" href="'.base_url('membre/Carte_New/index_carte/'.$carte['ID_CARTE']).'"> Détail et Apercu</a> </li>
    ...
    public function get_client_affilie($id)
    {
    
       $query_principal = 'SELECT membre_carte_membre.ID_CARTE, membre_carte_membre.ID_CARTE_MEMBRE, membre_carte_membre.DEBUT_SUR_LA_CARTE, membre_membre.CNI, membre_carte_membre.FIN_SUR_LA_CARTE, membre_carte_membre.STATUS, membre_membre.ID_MEMBRE, membre_membre.NOM, membre_membre.PRENOM, syst_provinces.PROVINCE_NAME, syst_communes.COMMUNE_NAME, syst_groupe_sanguin.DESCRIPTION AS GroupeSanguin, masque_emploi.DESCRIPTION AS Emploi, membre_membre.DATE_NAISSANCE, syst_sexe.DESCRIPTION as SEXE FROM `membre_carte_membre` JOIN membre_membre ON membre_membre.ID_MEMBRE = membre_carte_membre.ID_MEMBRE JOIN syst_provinces ON syst_provinces.PROVINCE_ID = membre_membre.PROVINCE_ID JOIN syst_communes ON syst_communes.COMMUNE_ID = membre_membre.COMMUNE_ID JOIN syst_groupe_sanguin ON syst_groupe_sanguin.ID_GROUPE_SANGUIN = membre_membre.ID_GROUPE_SANGUIN JOIN masque_emploi ON masque_emploi.ID_EMPLOI = membre_membre.ID_EMPLOI JOIN syst_sexe ON syst_sexe.ID_SEXE = membre_membre.ID_SEXE WHERE ID_CARTE =' . $id;

      //  $news = $this->Model->getRequete(' '.$carte['ID_CARTE'].' ');
    ...
    $order_by = '';
    $search = !empty($_POST['search']['value']) ? (" AND (membre_membre.NOM  LIKE '%$var_search%' OR membre_membre.PRENOM  LIKE '%$var_search%')") : '';
    $order_column='';
    $order_column = array('membre_membre.NOM');
    $order_by = isset($_POST['order']) ? ' ORDER BY ' . $order_column[$_POST['order']['0']['column']] . '  ' . $_POST['order']['0']['dir'] : ' ORDER BY membre_membre.NOM ASC';
    ...
    foreach ($fetch_cov_frais as $row) {
        $post = array();
        // $post[] = $u++;
        $newDate = date("d-m-Y", strtotime($row->DATE_NAISSANCE));
        $newDatedebutn = date("d-m-Y", strtotime($row->DEBUT_SUR_LA_CARTE));
        $newDatefinn = date("d-m-Y", strtotime($row->FIN_SUR_LA_CARTE));
        $post[] = $row->NOM.' '.$row->PRENOM;
        $post[] = $row->CNI;
        $post[] = $newDate;
        $post[] = $row->SEXE;
        $post[] = $row->PROVINCE_NAME.' '.$row->COMMUNE_NAME;
        $post[] = $row->GroupeSanguin;
        $post[] = $newDatedebutn.' au '.$newDatefinn;
        $data[] = $post;
    }
    ...
    public function modificationduree()
    {
      $ID_CARTE = $this->input->post('id');
    $result = $ID_CARTE;
    // $carte = $this->Model->getRequeteOne('Select * from membre_carte_membre WHERE ID_CARTE = '.$selected['ID_CARTE'].' limit 1'); 
    $carte=$this->Model->getRequeteOne('SELECT membre_carte.ID_CARTE, membre_carte.CODE_CARTE, membre_carte.DATE_DABUT_VALIDITE, membre_carte.DATE_FIN_VALIDITE FROM membre_carte JOIN membre_carte_membre ON membre_carte_membre.ID_CARTE = membre_carte.ID_CARTE WHERE membre_carte_membre.ID_CARTE = '.$ID_CARTE.' ');

    $result =' <form id="FormData" action="'.base_url().'membre/Carte_New/changedate" method="POST" enctype="multipart/form-data">
            <div class="modal-body">
...
// exit();
    ...
    $this->session->set_flashdata(array('message'=>$message));
      redirect(base_url('membre/Carte_New'));  
    ...
    $this->session->set_flashdata(array('message'=>$message));
      redirect(base_url('membre/Carte_New/index_carte/'.$ID_CARTE));  
    }
