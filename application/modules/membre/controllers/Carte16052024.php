...
class Carte extends CI_Controller {
     ...
     public function listing()
    {
      ...
      if ($ID_GROUPE != null) {
        ...
        $data['ID_GROUPE'] = $groupe['ID_GROUPE'];
      }

      $resultat=$this->Model->getRequete('SELECT DISTINCT(membre_membre.ID_MEMBRE), membre_membre.NOM, membre_membre.PRENOM, membre_membre.CNI, membre_carte.STATUS, membre_carte.ID_CARTE, membre_carte.CODE_CARTE, membre_carte.DATE_DABUT_VALIDITE, membre_carte.DATE_FIN_VALIDITE FROM membre_membre LEFT JOIN membre_carte_membre ON membre_carte_membre.ID_MEMBRE = membre_membre.ID_MEMBRE JOIN membre_carte ON membre_carte.ID_CARTE = membre_carte_membre.ID_CARTE AND membre_carte.STATUS = 1 JOIN membre_groupe_membre ON membre_groupe_membre.ID_MEMBRE = membre_membre.ID_MEMBRE JOIN membre_membre_qr ON membre_membre_qr.ID_MEMBRE = membre_membre.ID_MEMBRE WHERE membre_membre.IS_AFFILIE = 0 '.$condi2.' '.$condi5.' '.$condi1.' GROUP BY membre_membre.ID_MEMBRE, membre_carte.STATUS, membre_carte.ID_CARTE,membre_carte.CODE_CARTE, membre_carte.DATE_DABUT_VALIDITE, membre_carte.DATE_FIN_VALIDITE');
    ...
    //  exit();
      ...
      foreach ($resultat as $key) 
         {
          ...
          $nban = $this->Model->getRequeteOne('SELECT count(*) AS NB_AYANT FROM `membre_carte_membre` WHERE ID_CARTE = '.$key['ID_CARTE'].' ');
          // $person = $this->Model->getRequeteOne('SELECT NOM, PRENOM FROM membre_carte_membre JOIN membre_membre on membre_carte_membre.ID_MEMBRE = membre_membre.ID_MEMBRE WHERE membre_membre.IS_AFFILIE = 0 AND membre_carte_membre.ID_CARTE  = '.$key['ID_CARTE'].' ');
          
          $chambr=array();
          $chambr[]=$key['NOM'].' '.$key['PRENOM'];
          $chambr[]=$key['CODE_CARTE'];
          // newDate
          $newDatedebut = date("d-m-Y", strtotime($key['DATE_DABUT_VALIDITE']));
          $newDatefin = date("d-m-Y", strtotime($key['DATE_FIN_VALIDITE']));
          $chambr[]=$newDatedebut;  
          $chambr[]=$newDatefin;  
      ...
      <div class="modal fade" id="changedate'.$key['ID_CARTE'].'">
        <div class="modal-dialog modal-xl">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title">Membre de la carte '.$key['CODE_CARTE'].'</h4>
          ...
          <tr>
          <td>
          <input type="date" class="form-control" id="DEBUT_SUR_LA_CARTE" name="DEBUT_SUR_LA_CARTE" value="'.$key['DATE_DABUT_VALIDITE'].'" />
          </td>
          <td>
          <input type="date" class="form-control" id="FIN_SUR_LA_CARTE" name="FIN_SUR_LA_CARTE" value="'.$key['DATE_FIN_VALIDITE'].'" />
          <input type="hidden" class="form-control" id="ID_CARTE" name="ID_CARTE" value="'.$key['ID_CARTE'].'" />
          </td>
          </tr>
          </table>
          ...
          <div class="dropdown ">
                    ...
                    <li><a class="dropdown-item" href="'.base_url('membre/Carte/index_carte/'.$key['ID_CARTE']).'"> Détail et Apercu</a> </li>
                    <li><a class="dropdown-item" href="#" data-toggle="modal" data-target="#changedate'.$key['ID_CARTE'].'">Modifier la date de fin</a> </li>
                    </ul>
                  </div>';
       ...
       $tabledata[]=$chambr;
        ...
        $this->table->set_heading(array('Affili&eacute;','Numero','Debut','Fin','Nb membre','Etat','Option'));
